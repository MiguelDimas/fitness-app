        ...
        {ALL_BADGES.map((b) => {
          ...
          return (
            <div key={b.id} title={b.description}
              className={`flex flex-col items-center rounded-xl border p-3 text-center transition ${has ? "border-blue-200 bg-blue-50" : "border-gray-200 bg-gray-50"}`}>
              <span className={`text-2xl ${has ? "" : "opacity-40 grayscale"}`}>{b.emoji}</span>
              <span className={`mt-1 text-xs font-medium ${has ? "text-gray-700" : "text-gray-400"}`}>{b.name}</span>
              {has ? (
                <span className="mt-1 text-[10px] font-medium text-blue-600">Earned ✓</span>
              ) : (
